...
export function validateStyle(source: string, options: ParserOptions): string {
    let retString = "";
    while (source) {
        let matches = options.compiledStyles.exec(source);
        if (matches) {
...
export function validateClass(source: string, options: ParserOptions): string {
    let retString = "";
    while (source) {
        let matches = options.compiledClasses.exec(source);
        if (matches) {
            source = source.substring(matches[0].length);
            if (matches[1]) {
 ...
 */
export function humanReadable(options: ParserOptions): string {
